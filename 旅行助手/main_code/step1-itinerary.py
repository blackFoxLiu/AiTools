...
def process_record(record: Dict, client: OpenAI,
                   prompt_tools: str, prompt_rewrite: str) -> Optional[Dict]:
    """
    处理单条记录
    ...
    # 第一步：工具分析
    tools_response = call_model(client, prompt_tools, desc)
    if not tools_response:
        return None

    tools_data = safe_json_parse(tools_response)
    if not tools_data:
        return None

    # 补充原始 desc 字段
    tools_data["desc"] = desc
    ...
    # 第二步：优化重写
    rewrite_input = json.dumps(tools_data, ensure_ascii=False)
    rewrite_response = call_model(client, prompt_rewrite, rewrite_input)
    if not rewrite_response:
        return None

    rewrite_data = safe_json_parse(rewrite_response)
    if not rewrite_data:
        return None

    # 补充额外字段
    rewrite_data["node_id"] = record.get("note_id", "")
    rewrite_data["desc"] = desc

    # 验证
    if not checkJson_travel_tools(rewrite_data):
        print("验证失败，跳过")
        return None
...
def main():
    ...
    parser.add_argument("--input", "-i", default=crawler_file_path,
                        help="输入JSON文件路径")
    parser.add_argument("--prompt-tools", "-pt", default=travel_tools_v1_prompt_path,
                        help="交通工具分析提示词文件路径")
    parser.add_argument("--prompt-rewrite", "-pr", default=travel_tools_v2_prompt_path,
                        help="数据优化提示词文件路径")
    parser.add_argument("--output", "-o", default=itinerary_output_path,
                        help="输出文件路径（默认自动生成）")
    ...
    # 读取提示词
    prompt_tools = read_prompt(args.prompt_tools)
    prompt_rewrite = read_prompt(args.prompt_rewrite)
    if not prompt_tools or not prompt_rewrite:
        print("错误：无法读取提示词文件，请检查路径")
        return
    ...
    for record in tqdm(data, desc="处理进度"):
        try:
            result = process_record(record, client, prompt_tools, prompt_rewrite)
            if result:
